...
inline void TDecodeVertex(D *pOut, const uint8_t *pbIn) {
  ...
  const typename T::Input *const pIn = reinterpret_cast<const typename T::Input *>(pbIn);

  if constexpr (T::Input::DIM >= 1) { pOut->x = Math::TCast<floatf>(pIn->x); }
  else {
    if constexpr (D::DIM >= 1) { pOut->x = fZERO; }
  }

  if constexpr (T::Input::DIM >= 2) { pOut->y = Math::TCast<floatf>(pIn->y); }
  else {
    if constexpr (D::DIM >= 2) { pOut->y = fZERO; }
  }

  if constexpr (T::Input::DIM >= 3) { pOut->z = Math::TCast<floatf>(pIn->z); }
  else {
    if constexpr (D::DIM >= 3) { pOut->z = fZERO; }
  }

  if constexpr (T::Input::DIM >= 4) { pOut->w = Math::TCast<floatf>(pIn->w); }
  else {
    if constexpr (D::DIM >= 4) { pOut->w = fONE; }
  }
}
...
template <>
inline void TDecodeVertex<VECTOR4, TVertexData<VERTEX_RGBA>>(VECTOR4 *pOut,
                                                             const uint8_t *pbIn) {
  ASSERT(pOut);
  ASSERT(pbIn);
...
inline void CTNL::TProcessLighting_OneSided(unsigned count) {
  ...
  for (unsigned i = 0; i < count; ++i) {
    ...
    // Set the front color
    pcFrontColors[i].b = static_cast<uint8_t>(Math::TToUNORM8(vResult.x));
    pcFrontColors[i].g = static_cast<uint8_t>(Math::TToUNORM8(vResult.y));
    pcFrontColors[i].r = static_cast<uint8_t>(Math::TToUNORM8(vResult.z));
...
inline void CTNL::TProcessLighting_TwoSided(unsigned count) {
  ...
  for (unsigned i = 0; i < count; ++i) {
    ...
    // Set the front color
    pcFrontColors[i].b = static_cast<uint8_t>(Math::TToUNORM8(vResults[0].x));
    pcFrontColors[i].g = static_cast<uint8_t>(Math::TToUNORM8(vResults[0].y));
    pcFrontColors[i].r = static_cast<uint8_t>(Math::TToUNORM8(vResults[0].z));
    pcFrontColors[i].a = static_cast<uint8_t>(Math::TToUNORM8(vResults[0].w));

    // Set the back color
    pcBackColors[i].b = static_cast<uint8_t>(Math::TToUNORM8(vResults[1].x));
    pcBackColors[i].g = static_cast<uint8_t>(Math::TToUNORM8(vResults[1].y));
    pcBackColors[i].r = static_cast<uint8_t>(Math::TToUNORM8(vResults[1].z));
...
void CTNL::TProcessVertexColor(unsigned count) {
  ...
  for (unsigned i = 0; i < count; ++i) {
    ...
    // Set the front color
    pcFrontColors[i].b = static_cast<uint8_t>(Math::TToUNORM8(vColor.x));
    pcFrontColors[i].g = static_cast<uint8_t>(Math::TToUNORM8(vColor.y));
    pcFrontColors[i].r = static_cast<uint8_t>(Math::TToUNORM8(vColor.z));
...
void CTNL::TProcessTexCoords(unsigned dstIndex, unsigned srcIndex,
  ...
  for (unsigned i = 0; i < count; ++i) {
    ...
    TDecodeVertex<typename TVertexData<VertexFormat>::Output, TVertexData<VertexFormat>>(
        &vIn, pbIn + i * stride);

    if constexpr (Transform) { Math::Mul(&vIn, vIn, transform); }
