...
void TWriteColor(const RasterData &rasterData, const Color4 &cColor,
                 uint8_t *pCB) {
  unsigned dstColor = *reinterpret_cast<typename TFormatInfo<Format>::TYPE *>(pCB);
...
public:
  ...
  inline static void __Execute(const RasterData &rasterData,
                                             int y, int lx, int rx) {
    ...
    if constexpr (Depth) {
      fZdA = rasterData.Registers[REG_DEPTH + 0].fM[0];
      const fixedRX fZdB = rasterData.Registers[REG_DEPTH + 0].fM[1];
      const fixedRX fZdC = rasterData.Registers[REG_DEPTH + 0].fM[2];
    ...
    if constexpr (Color) {
      ...
      const fixedRX fBdC = rasterData.Registers[REG_COLOR + 0].fM[2];
      ...
      const fixedRX fGdC = rasterData.Registers[REG_COLOR + 1].fM[2];
      ...
      const fixedRX fRdC = rasterData.Registers[REG_COLOR + 2].fM[2];
      ...
      const fixedRX fAdC = rasterData.Registers[REG_COLOR + 3].fM[2];
    ...
    if constexpr (Texture0) {
      ...
      const fixedRX fU0dB = rasterData.Registers[REG_TEX0 + 0].fM[1];
      const fixedRX fU0dC = rasterData.Registers[REG_TEX0 + 0].fM[2];
      fU0 = fU0dA * fOffsetX + fU0dB * fOffsetY + fU0dC;

      fV0dA = rasterData.Registers[REG_TEX0 + 1].fM[0];
      const fixedRX fV0dB = rasterData.Registers[REG_TEX0 + 1].fM[1];
      const fixedRX fV0dC = rasterData.Registers[REG_TEX0 + 1].fM[2];
      ...
      if constexpr (Texture1) {
        ...
        const fixedRX fU1dB = rasterData.Registers[REG_TEX1 + 0].fM[1];
        const fixedRX fU1dC = rasterData.Registers[REG_TEX1 + 0].fM[2];
        fU1 = fU1dA * fOffsetX + fU1dB * fOffsetY + fU1dC;

        fV1dA = rasterData.Registers[REG_TEX1 + 1].fM[0];
        const fixedRX fV1dB = rasterData.Registers[REG_TEX1 + 1].fM[1];
        const fixedRX fV1dC = rasterData.Registers[REG_TEX1 + 1].fM[2];
    ...
    if constexpr (Fog) {
      fFogdA = rasterData.Registers[REG_FOG + 0].fM[0];
      const fixedRX fFogdB = rasterData.Registers[REG_FOG + 0].fM[1];
      const fixedRX fFogdC = rasterData.Registers[REG_FOG + 0].fM[2];
    ...
    do {
      for (;;) {
        ...
        if constexpr (Color) { ToColor4(&cColor, fA, fR, fG, fB); }
...
public:
  ...
  inline static void __Execute(const RasterData &rasterData,
    ...
    if constexpr (Texture0) {
      fRhwdA = rasterData.Registers[REG_RHW + 0].fM[0];
      const fixedRX fRhwdB = rasterData.Registers[REG_RHW + 0].fM[1];
      const fixedRX fRhwdC = rasterData.Registers[REG_RHW + 0].fM[2];
      ...
      if constexpr (2 == Texture0) {
        fM0OverW2dA = rasterData.Registers[REG_MIP0 + 0].fM[0];
        fM0OverW2 = rasterData.Registers[REG_MIP0 + 0].fM[2];

        if (rasterFlags.InterpolateMips & 1) {
          const fixedRX fM0OverW2dB = rasterData.Registers[REG_MIP0 + 0].fM[1];
      ...
      if constexpr (Texture1) {
        ...
        if constexpr (2 == Texture1) {
          fM1OverW2dA = rasterData.Registers[REG_MIP1 + 0].fM[0];
          fM1OverW2 = rasterData.Registers[REG_MIP1 + 0].fM[2];

          if (rasterFlags.InterpolateMips & 2) {
            const fixedRX fM1OverW2dB = rasterData.Registers[REG_MIP1 + 0].fM[1];
    ...
    if constexpr ((2 == Texture0) || (2 == Texture1)) { fW2 = fW * fW; }
    ...
    do {
      ...
      if (log2width) {
        ...
        if constexpr ((2 == Texture0) || (2 == Texture1)) { fWr2 = fWr * fWr; }
      ...
      do {
        ...
        if constexpr (Texture0) {
          ...
          if constexpr (2 == Texture0) { fM0 += fdMr0; }
        }
        ...
        if constexpr (Texture1) {
          ...
          if constexpr (2 == Texture1) { fM1 += fdMr1; }
        }
