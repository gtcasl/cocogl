...
bool TStencilTest(unsigned depthValue, unsigned stencilRef,
  ...
  if constexpr (StencilWrite) { writeMask = stencilWriteMask << 16; }

  bool bStencilTest = TCompare<StencilFunc>(_stencilRef, _stencilValue);
  if (bStencilTest) {
    if (TCompare<(DepthTest ? DepthFunc : static_cast<uint32_t>(COMPARE_ALWAYS))> (depthValue, _depthValue)) {
      stencilResult = TStencilOp<StencilPass>(stencilValue, stencilRef);

      if constexpr (DepthTest && DepthWrite) { writeMask |= 0xffff; }
    } else {
...
void TGetTexEnvColor(Color4 *pInOut, unsigned texture, ColorARGB cEnvColor) {
  ...
  if constexpr (NativeColor) {
    if constexpr (EnvMode == ENVMODE_REPLACE) {
      ...
      if constexpr (FormatSize::RGB) { pInOut->b = texture; }
...
private:
  ...
  inline static unsigned GetBlendOp(unsigned inColor, unsigned srcColor,
    ...
    if constexpr (BlendOp == BLEND_SRC_ALPHA) {
      __unreferenced(srcColor);
      __unreferenced(dstColor);
      const unsigned alpha = srcAlpha >> (8 - TFormatInfo<FORMAT_R5G6B5>::LERP);
...
public:
  ...
  inline static unsigned Execute(const Color4 &cColor, unsigned dstColor) {
    if constexpr (NativeBlend) {
      ...
      if constexpr (NativeColor) { srcColor = cColor.b; }
      else {
      ...
      if constexpr (NativeColor) {
        Format::TConvertFromNative<FORMAT_R5G6B5, false>(&cSrcColor, cColor.b);
        cSrcColor.a = cColor.a;
      }
      else { cSrcColor = cColor; }
...
public:
  ...
  inline  static void Execute(const RasterData &rasterData, int y,
    ...
    if constexpr (DepthTest) {
      fZ = rasterData.Registers[REG_DEPTH].fM[2];

      if constexpr (InterpolateDepth) {
        fZdA = rasterData.Registers[REG_DEPTH].fM[0];
        fixedRX fZdB = rasterData.Registers[REG_DEPTH].fM[1];
    ...
    if constexpr (Color) {
      fB = rasterData.Registers[REG_COLOR + 0].fM[2];
      fG = rasterData.Registers[REG_COLOR + 1].fM[2];
      fR = rasterData.Registers[REG_COLOR + 2].fM[2];
      fA = rasterData.Registers[REG_COLOR + 3].fM[2];

      if constexpr (InterpolateColor) {
        fBdA = rasterData.Registers[REG_COLOR + 0].fM[0];
        fixedRX fBdB = rasterData.Registers[REG_COLOR + 0].fM[1];
        fB += fBdA * fOffsetX + fBdB * fOffsetY;

        fGdA = rasterData.Registers[REG_COLOR + 1].fM[0];
        fixedRX fGdB = rasterData.Registers[REG_COLOR + 1].fM[1];
        fG += fGdA * fOffsetX + fGdB * fOffsetY;

        fRdA = rasterData.Registers[REG_COLOR + 2].fM[0];
        fixedRX fRdB = rasterData.Registers[REG_COLOR + 2].fM[1];
        fR += fRdA * fOffsetX + fRdB * fOffsetY;
      }

      if constexpr (InterpolateAlpha) {
        fAdA = rasterData.Registers[REG_COLOR + 3].fM[0];
        fixedRX fAdB = rasterData.Registers[REG_COLOR + 3].fM[1];
        fA += fAdA * fOffsetX + fAdB * fOffsetY;
      }

      if constexpr (NativeColor) {        
        ToColor4(&cNativeColor, fA, fR, fG, fB);
        cNativeColor.b = Format::TConvertToNative<ColorFormat>(cNativeColor);
      }
    }
    ...
    if constexpr (Texture0) {
      fU0dA = rasterData.Registers[REG_TEX0 + 0].fM[0];
      fixedRX fU0dB = rasterData.Registers[REG_TEX0 + 0].fM[1];
      fixedRX fU0dC = rasterData.Registers[REG_TEX0 + 0].fM[2];
      fU0 = fU0dA * fOffsetX + fU0dB * fOffsetY + fU0dC;

      fV0dA = rasterData.Registers[REG_TEX0 + 1].fM[0];
      fixedRX fV0dB = rasterData.Registers[REG_TEX0 + 1].fM[1];
      fixedRX fV0dC = rasterData.Registers[REG_TEX0 + 1].fM[2];
      fV0 = fV0dA * fOffsetX + fV0dB * fOffsetY + fV0dC;

      if constexpr (Mip0) {
        fM0 = rasterData.Registers[REG_MIP0 + 0].fM[2];
      }

      if constexpr (Texture1) {
        fU1dA = rasterData.Registers[REG_TEX1 + 0].fM[0];
        fixedRX fU1dB = rasterData.Registers[REG_TEX1 + 0].fM[1];
        fixedRX fU1dC = rasterData.Registers[REG_TEX1 + 0].fM[2];
        fU1 = fU1dA * fOffsetX + fU1dB * fOffsetY + fU1dC;

        fV1dA = rasterData.Registers[REG_TEX1 + 1].fM[0];
        fixedRX fV1dB = rasterData.Registers[REG_TEX1 + 1].fM[1];
        fixedRX fV1dC = rasterData.Registers[REG_TEX1 + 1].fM[2];
        fV1 = fV1dA * fOffsetX + fV1dB * fOffsetY + fV1dC;

        if constexpr (Mip1) {
          fM1 = rasterData.Registers[REG_MIP1 + 0].fM[2];
        }
      }
    }
    ...
    if constexpr (Fog) {
      fFog = rasterData.Registers[REG_FOG + 0].fM[2];

      if constexpr (InterpolateFog) {
        fFogdA = rasterData.Registers[REG_FOG].fM[0];
        fixedRX fFogdB = rasterData.Registers[REG_FOG].fM[1];
    ...
    do {
      for (;;) {
        ...
        if constexpr (Color) {
          if constexpr (NativeColor) { cColor = cNativeColor; }
          else {
        ...
        if constexpr (Blend) {
          ...
          if constexpr (NativeColor) { color = cColor.b; }
          else {
        ...
        // Write output color
        TWriteColor<ColorFormat, (LogicOp ? LogicFunc : static_cast<uint32_t>(LOGICOP_COPY)),
                    ColorWriteMask>(color, dstColor, colorWriteMask, pCB);
        break;
      }

      if constexpr (DepthTest || StencilTest) { pDS += DepthStencilStride; }

      if constexpr (InterpolateDepth) { fZ += fZdA; }
      ...
      if constexpr (InterpolateAlpha) { fA += fAdA; }
      ...
      if constexpr (InterpolateFog) { fFog += fFogdA; }
...
public:
  ...
  inline  static void Execute(const RasterData &rasterData, int y,
    ...
    if constexpr (Texture0) {
      fRhwdA = rasterData.Registers[REG_RHW].fM[0];
      fixedRX fRhwdB = rasterData.Registers[REG_RHW].fM[1];
      fixedRX fRhwdC = rasterData.Registers[REG_RHW].fM[2];
      fRhw = fRhwdA * fOffsetX + fRhwdB * fOffsetY + fRhwdC;

      fU0OverWdA = rasterData.Registers[REG_TEX0 + 0].fM[0];
      fixedRX fU0OverWdB = rasterData.Registers[REG_TEX0 + 0].fM[1];
      fixedRX fU0OverWdC = rasterData.Registers[REG_TEX0 + 0].fM[2];
      fU0OverW =
          fU0OverWdA * fOffsetX + fU0OverWdB * fOffsetY + fU0OverWdC;

      fV0OverWdA = rasterData.Registers[REG_TEX0 + 1].fM[0];
      fixedRX fV0OverWdB = rasterData.Registers[REG_TEX0 + 1].fM[1];
      fixedRX fV0OverWdC = rasterData.Registers[REG_TEX0 + 1].fM[2];
      fV0OverW =
          fV0OverWdA * fOffsetX + fV0OverWdB * fOffsetY + fV0OverWdC;

      if constexpr (Mip0) {
        fM0OverW2 = rasterData.Registers[REG_MIP0].fM[2];

        if constexpr (InterpolateMips & 0x1) {
          fM0OverW2dA = rasterData.Registers[REG_MIP0].fM[0];
          fixedRX fM0OverW2dB = rasterData.Registers[REG_MIP0].fM[1];
          fM0OverW2 += fM0OverW2dA * fOffsetX + fM0OverW2dB * fOffsetY;
        }
      }

      if constexpr (Texture1) {
        fixedRX fU1OverWdA = rasterData.Registers[REG_TEX1 + 0].fM[0];
        fixedRX fU1OverWdB = rasterData.Registers[REG_TEX1 + 0].fM[1];
        fixedRX fU1OverWdC = rasterData.Registers[REG_TEX1 + 0].fM[2];
        fU1OverW =
            fU1OverWdA * fOffsetX + fU1OverWdB * fOffsetY + fU1OverWdC;

        fixedRX fV1OverWdA = rasterData.Registers[REG_TEX1 + 1].fM[0];
        fixedRX fV1OverWdB = rasterData.Registers[REG_TEX1 + 1].fM[1];
        fixedRX fV1OverWdC = rasterData.Registers[REG_TEX1 + 1].fM[2];
        fV1OverW =
            fV1OverWdA * fOffsetX + fV1OverWdB * fOffsetY + fV1OverWdC;

        if constexpr (Mip1) {
          fM1OverW2 = rasterData.Registers[REG_MIP1].fM[2];

          if constexpr (InterpolateMips & 0x2) {
            fM1OverW2dA = rasterData.Registers[REG_MIP1].fM[0];
            fixedRX fM1OverW2dB = rasterData.Registers[REG_MIP1].fM[1];
    ...
    if constexpr (Mip0 || Mip1) { fW2 = fW * fW; }
    ...
    do {
      ...
      do {
        ...
        if constexpr (Texture0) {
          ...
          if constexpr (InterpolateMips & 0x1) { fM0 += fdMr0; }
        }
        ...
        if constexpr (Texture1) {
          ...
          if constexpr (InterpolateMips & 0x2) { fM1 += fdMr1; }
        }
...
public:
  ...
  inline  static void Execute(const RasterData &rasterData, int y,
    ...
    do {
      ...
      if constexpr (InterpolateAlpha) { fA += fAdA * offset; }
      ...
      if constexpr (InterpolateFog) { fFog += fFogdA * offset; }
...
public:
  ...
  inline  static void Execute(const RasterData &rasterData, int y,
    ...
    if constexpr (DepthTest) {
      ...
      if constexpr (InterpolateDepth) {
        fZdA = rasterData.Registers[REG_DEPTH].fM[0];
        const fixedRX fZdB = rasterData.Registers[REG_DEPTH].fM[1];
        fZ += fZdA * fOffsetX + fZdB * fOffsetY;
      }
    }
    ...
    if constexpr (Color) {
      ...
      if constexpr (InterpolateColor) {
        fBdA = rasterData.Registers[REG_COLOR + 0].fM[0];
        const fixedRX fBdB = rasterData.Registers[REG_COLOR + 0].fM[1];
        fB += fBdA * fOffsetX + fBdB * fOffsetY;

        fGdA = rasterData.Registers[REG_COLOR + 1].fM[0];
        const fixedRX fGdB = rasterData.Registers[REG_COLOR + 1].fM[1];
        fG += fGdA * fOffsetX + fGdB * fOffsetY;

        fRdA = rasterData.Registers[REG_COLOR + 2].fM[0];
        const fixedRX fRdB = rasterData.Registers[REG_COLOR + 2].fM[1];
        fR += fRdA * fOffsetX + fRdB * fOffsetY;
      }

      if constexpr (InterpolateAlpha) {
        fAdA = rasterData.Registers[REG_COLOR + 3].fM[0];
        const fixedRX fAdB = rasterData.Registers[REG_COLOR + 3].fM[1];
        fA += fAdA * fOffsetX + fAdB * fOffsetY;
      }
    ...
    if constexpr (Texture0) {
      const fixedRX fRhwdA = rasterData.Registers[REG_RHW].fM[0];
      const fixedRX fRhwdB = rasterData.Registers[REG_RHW].fM[1];
      const fixedRX fRhwdC = rasterData.Registers[REG_RHW].fM[2];
      fRhw = fRhwdA * fOffsetX + fRhwdB * fOffsetY + fRhwdC;

      fU0OverWdA = rasterData.Registers[REG_TEX0 + 0].fM[0];
      const fixedRX fU0OverWdB = rasterData.Registers[REG_TEX0 + 0].fM[1];
      const fixedRX fU0OverWdC = rasterData.Registers[REG_TEX0 + 0].fM[2];
      fU0OverW =
          fU0OverWdA * fOffsetX + fU0OverWdB * fOffsetY + fU0OverWdC;

      fV0OverWdA = rasterData.Registers[REG_TEX0 + 1].fM[0];
      const fixedRX fV0OverWdB = rasterData.Registers[REG_TEX0 + 1].fM[1];
      const fixedRX fV0OverWdC = rasterData.Registers[REG_TEX0 + 1].fM[2];
      fV0OverW =
          fV0OverWdA * fOffsetX + fV0OverWdB * fOffsetY + fV0OverWdC;
      ...
      if constexpr (Mip0) {
        ...
        if constexpr (InterpolateMips & 0x1) {
          fM0OverW2dA = rasterData.Registers[REG_MIP0].fM[0];
          const fixedRX fM0OverW2dB = rasterData.Registers[REG_MIP0].fM[1];
          fM0OverW2 += fM0OverW2dA * fOffsetX + fM0OverW2dB * fOffsetY;
        }
      }

      if constexpr (Texture1) {
        fU1OverWdA = rasterData.Registers[REG_TEX1 + 0].fM[0];
        const fixedRX fU1OverWdB = rasterData.Registers[REG_TEX1 + 0].fM[1];
        const fixedRX fU1OverWdC = rasterData.Registers[REG_TEX1 + 0].fM[2];
        fixedRX fU1OverW =
            fU1OverWdA * fOffsetX + fU1OverWdB * fOffsetY + fU1OverWdC;

        fV1OverWdA = rasterData.Registers[REG_TEX1 + 1].fM[0];
        const fixedRX fV1OverWdB = rasterData.Registers[REG_TEX1 + 1].fM[1];
        const fixedRX fV1OverWdC = rasterData.Registers[REG_TEX1 + 1].fM[2];
        fixedRX fV1OverW =
            fV1OverWdA * fOffsetX + fV1OverWdB * fOffsetY + fV1OverWdC;
        ...
        if constexpr (Mip1) {
          ...
          if constexpr (InterpolateMips & 0x2) {
            fM1OverW2dA = rasterData.Registers[REG_MIP1].fM[0];
            const fixedRX fM1OverW2dB = rasterData.Registers[REG_MIP1].fM[1];
            fM1OverW2 += fM1OverW2dA * fOffsetX + fM1OverW2dB * fOffsetY;
          }
        }
      }
    ...
    if constexpr (Fog) {
      ...
      if constexpr (InterpolateFog) {
        fFogdA = rasterData.Registers[REG_FOG].fM[0];
        const fixedRX fFogdB = rasterData.Registers[REG_FOG].fM[1];
        fFog += fFogdA * fOffsetX + fFogdB * fOffsetY;
      }
    }
...
public:
   static void Execute(const RasterData &rasterData, int y, int lx,
                                    int rx) {
